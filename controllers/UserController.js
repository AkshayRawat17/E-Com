...
async function createRecord(req, res) {
  if (schema.validate(req.body.password)) {
        ...
        try {
          ...
          error.keyValue?.username
            ? (errorMessage.username =
                "User with this User Name is Already Exists")
            : null;
    ...
    res.status(400).send({
      result: "Fail",
      reason:
        "Invalid Password.It must contains at least 1 upper case, 1 lower case letter,and 1 digit, should not contain any space and length must 8-100 characters",
    });
  }
}
...
async function updateRecord(req, res) {
  try {
    ...
    let errorMessage = {};
    error.keyValue?.username
      ? (errorMessage.username = "User with this User name is Already Exists")
      : null;
    error.keyValue?.email
      ? (errorMessage.email = "User with this Email Address is Already Exists")
      : null;
  ...
  try {
    ...
    if (data) {
      if (await bcrypt.compare(req.body.password, data.password)) {
        ...
        jwt.sign({ data }, key, { expiresIn: "20 Days" }, (error, token) => {
          if (error)
            res.status(500).send({
              result: "Fail",
              reason: "Internal Server Error",
